package doext.swipe.pullToRefresh;
...
public class DoPullToRefreshTools {
	...
	public ImageView foot_ImageView;
	public TextView foot_TextView;
	...
	public RelativeLayout getPullToRefreshViewFoot(String typeID) {

		DoIModuleTypeID id = new GetTypeID(typeID);

		int footview_id = DoResourcesHelper.getIdentifier("footview", "layout", id);
		RelativeLayout footView = (RelativeLayout) View.inflate(ctx, footview_id, null);
		footView.setLayoutParams(new LayoutParams(-1, -2));

		int tv_state_id = DoResourcesHelper.getIdentifier("tv_state", "id", id);
		this.foot_TextView = (TextView) footView.findViewById(tv_state_id);
		this.foot_TextView.setText("上拉加载更多");

		int pb_id = DoResourcesHelper.getIdentifier("pb", "id", id);
		this.foot_ProgressBar = (ProgressBar) footView.findViewById(pb_id);

		int iv_id = DoResourcesHelper.getIdentifier("iv", "id", id);
		this.foot_ImageView = (ImageView) footView.findViewById(iv_id);
		int pulltorefresh_arrow_id = DoResourcesHelper.getIdentifier("pulltorefresh_arrow_up", "drawable", id);
		this.foot_ImageView.setImageBitmap(BitmapFactory.decodeResource(ctx.getResources(), pulltorefresh_arrow_id));
